...
private val connectionLightPrimaryColorKey = intPreferencesKey("connectionLightPrimaryColor")
private val connectionLightSecondaryColorKey = intPreferencesKey("connectionLightSecondaryColor")
private val connectionLightGradientEnabledKey = booleanPreferencesKey("connectionLightGradientEnabled")
...
class DefaultAppPreferencesStore(
    ...
    private val defaultConnectionLightPrimaryColor = 0xFFA8D8FF.toInt()
    private val defaultConnectionLightSecondaryColor = 0xFFF0F4FF.toInt()
    ...
    override suspend fun setConnectionLightSettings(primaryColorArgb: Int, secondaryColorArgb: Int, gradientEnabled: Boolean) {
        store.edit { prefs ->
            prefs[connectionLightPrimaryColorKey] = primaryColorArgb
            prefs[connectionLightSecondaryColorKey] = secondaryColorArgb
            prefs[connectionLightGradientEnabledKey] = gradientEnabled
        }
    }

    override fun getConnectionLightPrimaryColorFlow(): Flow<Int> {
        return store.data.map { prefs ->
            prefs[connectionLightPrimaryColorKey] ?: defaultConnectionLightPrimaryColor
        }
    }

    override fun getConnectionLightSecondaryColorFlow(): Flow<Int> {
        return store.data.map { prefs ->
            prefs[connectionLightSecondaryColorKey] ?: defaultConnectionLightSecondaryColor
        }
    }

    override fun isConnectionLightGradientEnabledFlow(): Flow<Boolean> {
        return store.data.map { prefs ->
            prefs[connectionLightGradientEnabledKey] ?: false
        }
    }
